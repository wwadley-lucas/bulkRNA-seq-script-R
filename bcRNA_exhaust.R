...
paths <- list(
  project_dir  = "~/Desktop/Research/Graduate Research/Fleischman lab/Exhaustion Paper/Acute exposure mice /",
  counts_csv   = "Tables/All probes.csv",
  meta_csv     = "Tables/metaData.csv",
  meta_grouped = "Tables/metaData.csv",  # reuse if only one meta
  out_base     = "Figures/"
)
FIG <- list(W = 8, H = 6, DPI = 300)
...
dir_create <- function(...) invisible(lapply(list(...), function(d) dir.create(d, recursive = TRUE, showWarnings = FALSE)))
do.call(dir_create, outs)
...
gene_set <- msigdbr(species = species_msigdb, collection = "H", subcollection = NULL)
